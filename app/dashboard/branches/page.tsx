...
import { useCabangStore, type Cabang } from '@/stores/useCabangStore';
...
function BranchesContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let success = false;
    if (editingCabangId) {
      success = await updateCabang(editingCabangId, formData);
      if (success) alert('Cabang berhasil diupdate!');
    } else {
      success = await createCabang(formData);
      if (success) alert('Cabang berhasil ditambahkan!');
    }
  ...
  const handleToggleActive = async (cabang: Cabang) => {
    ...
    const success = await toggleCabangStatus(cabang.id, !cabang.isActive);
    if (success) {
  ...
  const handleDelete = async (cabang: Cabang) => {
    ...
    const success = await deleteCabang(cabang.id);
    if (success) {
    ...
    <div className="px-4 md:px-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <nav className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
          ...
          <span>›</span>
          <span className="text-gray-900 dark:text-white font-medium">Cabang</span>
        </nav>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
          <p className="text-2xl font-bold text-green-600">{cabangs.filter(c => c.isActive).length}</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="p-4 md:p-6 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">Daftar Cabang</h2>
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                {editingCabangId ? 'Edit Cabang' : 'Tambah Cabang Baru'}
              </h3>
              <button
